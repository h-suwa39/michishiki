...
(() => {
  ...
  /* 全部できたときの見出し。「/」の位置でだけ折り返す（語の途中や句点の前で切れないように） */
  const FINALE_TITLES = [
    'きょうも、/がんばってくれて/ありがとう。',
    'おつかれさま。/ここまで、/ちゃんと来ました。',
    'がんばったね。/きょうの分は、/おしまい。',
    'ぜんぶ歩けました。/もう休んでいい/時間です。',
    'きょうのあなたは、/ちゃんと歩きました。',
    'よくやりました。/あとは、ゆっくり。',
    'ひとつずつ、ぜんぶ。/すごいことです。',
    'きょうの道、/ぜんぶ歩ききりました。',
  ...
  const FINALE_TITLES_NIGHT = [
    ...
    'おやすみ。/がんばってくれて、/ありがとう。',
    'きょうはここまで。/ゆっくり眠って/ください。',
  ];
  const FINALE_TITLES_DAY = [
    'ひと息つきましょう。/きょうはもう、/上がりです。',
    'あとの時間は、/好きに使っていい/時間です。',
  ];
  /* 「/」区切りの句を、それぞれ折り返さない単位にして描く */
  const phrasesHtml = t => t.split('/').map(seg => `<span class="ph">${esc(seg)}</span>`).join('');
  const FINALE_SUB = [
    'きょうのあなたは、ちゃんと歩きました。',
    'ここまで来た自分に、ひと息。',
    'あとは、ゆっくり休んでください。',
  ...
  function finale() {
    ...
    $('#finaleTitle').innerHTML = phrasesHtml(pool[Math.floor(Math.random() * pool.length)]);
